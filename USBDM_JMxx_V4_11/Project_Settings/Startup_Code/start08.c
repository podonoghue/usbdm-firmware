...
/* main. This version does however NOT initialise global variables         */
...
/* purpose:     1) zero out RAM-areas where data is allocated
                ...
                3) copy initialisation data from ROM to RAM
 */
...
copydone:
  ...
  /* implement ROM libraries initialisation here (see startup.c) */
}
...
/* set the reset vector to _Startup in the linker parameter file (*.prm):
    ...
    purpose:    1)  initialise the stack
                2)  initialise run-time, ...
                    initialise the RAM, copy down init data, etc (Init)
                3)  call main;
