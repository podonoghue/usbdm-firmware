...
U8 f_CMD_CF_WRITE_REG(void) {
   BDMCF_CMD_WRITE_REG(commandBuffer[3]&0x1F,(*(U32 *)(commandBuffer+4)));
   return BDM_RC_OK;
}
...
U8 f_CMD_CF_READ_REG(void) {
   ...
   BDMCF_CMD_READ_REG(commandBuffer[3]&0x1F,commandBuffer+1);
   returnSize  = 5;
   return BDM_RC_OK;
}
...
U16 regNo = *(U16*)(commandBuffer+2);
   ...
   if (regNo >= CFV1_ByteRegs) {
      ...
      BDMCF_CMD_READ_DREG((U8)regNo&0x1F,commandBuffer+1);
#ifdef MC51AC256_HACK
      if (regNo == CFV1_DRegCSR)
...
U8 f_CMD_CF_WRITE_CREG(void) {
   BDMCF_CMD_WRITE_CREG(commandBuffer[3]&0x1F, (*(U32 *)(commandBuffer+4)));
   return BDM_RC_OK;
}
...
U8 f_CMD_CF_READ_CREG(void) {
   BDMCF_CMD_READ_CREG(commandBuffer[3]&0x1F, commandBuffer+1);
   returnSize = 5;
   return BDM_RC_OK;
}
