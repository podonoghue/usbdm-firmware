...
void fastTimerWait(U16 delay){
   IC_BDM_TIMING_TMOD = delay;           // time to wait
   IC_BDM_TIMING_TSC = FAST_T1_VALUE;    // reset the timer and start counting
   IC_BDM_TIMING_TSC_TOF = 0;            // clear TOF
   while (!WAIT_TIMER_EXPIRED) {                // Wait for timeout
   } 
}
...
void slowTimerWait(U16 delay){
   IC_BDM_TIMING_TMOD = delay;           // time to wait
   IC_BDM_TIMING_TSC = SLOW_T1_VALUE;    // reset the timer and start counting
   IC_BDM_TIMING_TSC_TOF = 0;            // clear TOF
   while (!WAIT_TIMER_EXPIRED) {
   }
}
...
U16 bdm_targetVddMeasure(void) {
   ...
   ADCSC1 = 5;        // Trigger single conversion on Ch9
