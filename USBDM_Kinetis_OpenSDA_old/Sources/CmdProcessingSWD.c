...
//!
USBDM_ErrorCode swd_writeMemoryWord(const uint8_t *address, const uint8_t *data) {
	USBDM_ErrorCode  rc;
	uint8_t  temp[4];
   /* Steps
...
//!  != \ref BDM_RC_OK => various errors
//!
USBDM_ErrorCode swd_readMemoryWord(const uint8_t *address, uint8_t *data) {
USBDM_ErrorCode  rc;
uint8_t  temp[4];
...
//!
static USBDM_ErrorCode swd_coreRegisterOperation(uint8_t *DCRSRvalue) {
 ...
 *  @return error code
 */
static USBDM_ErrorCode readCoreRegister(uint8_t regNo, uint8_t *outptr) {
   USBDM_ErrorCode rc;
   // Set up command
...
//!    != \ref BDM_RC_OK => error         \n
//!
static USBDM_ErrorCode modifyDHCSR(uint8_t preserveBits, uint8_t setBits) {
