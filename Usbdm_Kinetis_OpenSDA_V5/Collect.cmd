...
for %%f in (%FIRMWARE_VERSIONS%) do copy "%Kinetis_ROOT%\%%f\*.sx"        "%MK_FLASH_REFERENCE%"
...
for %%f in (%FIRMWARE_VERSIONS%) do copy /B "%Kinetis_ROOT%\%%f\*.bin"        "%MK_FLASH_REFERENCE%"
...
for %%f in (%BOARD%) do copy "%Kinetis_ROOT%\OpenSDAv1\*.sx"            %MK_FLASH_DIR%\%%f.sx
for %%f in (%BOARD%) do copy "%Kinetis_ROOT%\OpenSDAv1_Unique_ID\*.sx"  %MK_FLASH_DIR%\%%f_Unique_ID.sx

REM OpenSDA V1 + Power (P&E)
set BOARD=FRDM-KEAZ64Q64 FRDM-KEAZN32Q64 FRDM-KEAZ128Q80 S32K144EVM
for %%f in (%BOARD%) do copy "%Kinetis_ROOT%\OpenSDAv1_Power\*.sx"            %MK_FLASH_DIR%\%%f.sx
for %%f in (%BOARD%) do copy "%Kinetis_ROOT%\OpenSDAv1_Unique_ID_Power\*.sx"  %MK_FLASH_DIR%\%%f_Unique_ID.sx

REM OpenSDA V2
set BOARD=FRDM-KL82Z
for %%f in (%BOARD%) do copy /B "%Kinetis_ROOT%\OpenSDAv2_0\*.bin"            %MK_FLASH_DIR%\%%f.bin
